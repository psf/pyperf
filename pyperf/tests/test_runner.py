...
class TestRunner(unittest.TestCase):
    ...
    def test_worker(self):
        ...
        self.assertRegex(
            result.stdout, r"^bench: Mean \+- std dev: 1\.00 sec \+- 0\.00 sec\n$"
        )
    ...
    def test_pipe(self):
        ...
        with rpipe:
            with wpipe:
                ...
                result = self.exec_runner("--pipe", str(arg), "--worker", "-l1", "-w1")
    ...
    def test_pipe_with_timeout(self):
        ...
        with rpipe:
            ...
            with mock.patch(
                "pyperf._utils.select.select", return_value=(False, False, False)
            ):
                with self.assertRaises(TimeoutError) as cm:
                    rpipe.read_text(timeout=0.1)
                self.assertEqual(str(cm.exception), "Timed out after 0.1 seconds")

            # Mock the select to make the read pipeline ready
            with mock.patch(
                "pyperf._utils.select.select", return_value=(True, False, False)
            ):
                bench_json = rpipe.read_text(timeout=0.1)
                self.assertEqual(
                    bench_json.rstrip(), tests.benchmark_as_json(result.bench).rstrip()
                )
    ...
    def test_json_exists(self):
        with tempfile.NamedTemporaryFile("wb+") as tmp:
            with tests.capture_stdout() as stdout:
                try:
                    self.create_runner(["--worker", "-l1", "-w1", "--output", tmp.name])
    ...
    def test_verbose_metadata(self):
        result = self.exec_runner("--worker", "-l1", "-w1", "--verbose", "--metadata")
        self.assertRegex(
            result.stdout,
            r"^"
    ...
    def test_loops_calibration(self):
        ...
        result = self.exec_runner(
            "--worker", "--calibrate-loops", "-v", time_func=time_func
        )
    ...
    def test_loops_calibration_min_time(self):
        ...
        result = self.exec_runner(
            "--worker", "--calibrate-loops", "--min-time", "0.001", time_func=time_func
        )
        for run in result.bench.get_runs():
    ...
    def test_json_file(self):
        with tests.temporary_directory() as tmpdir:
            ...
            result = self.exec_runner("--worker", "-l1", "-w1", "--output", filename)
    ...
    def test_calibration_zero(self):
        ...
        with self.assertRaises(SystemExit):
            with tests.capture_stdout() as stdout:
                runner.bench_time_func("bench", time_func)
        self.assertIn(
            "ERROR: failed to calibrate the number of loops", stdout.getvalue()
        )
    ...
    def test_calibrate_loops(self):
        args = ["--worker", "-w0", "-n2", "--min-time=1.0", "--calibrate-loops"]
        runner = self.create_runner(args)
    ...
    def test_show_name(self):
        ...
        self.assertRegex(
            result.stdout, r"^NAME: Mean \+- std dev: 1\.00 sec \+- 0\.00 sec\n$"
        )

        result = self.exec_runner(
            "--worker", "-l1", "-w1", name="NAME", show_name=False
        )
        self.assertRegex(
            result.stdout, r"^Mean \+- std dev: 1\.00 sec \+- 0\.00 sec\n$"
        )
    ...
    def test_compare_to(self):
        ...
        run = pyperf.Run([1.5], metadata={"name": "name"}, collect_metadata=False)
        ...
        with ExitStack() as cm:
            ...
            cm.enter_context(
                mock.patch("pyperf._runner.abs_executable", side_effect=abs_executable)
            )
            cm.enter_context(
                mock.patch("pyperf._manager._load_suite_from_pipe", return_value=suite)
            )

            args = [
                "--python=python3.8",
                "--compare-to=python3.6",
                "--min-time=5",
                "-p1",
                "-w3",
                "-n7",
    ...
    def test_duplicated_named(self):
        ...
        self.assertEqual(str(cm.exception), "duplicated benchmark name: 'optim'")
    ...
    def test_bench_command(self):
        ...
        self.assertEqual(
            bench.get_metadata()["command"], " ".join(map(shell_quote, args))
        )
    ...
    def test_hook_command(self):
        ...
        self.assertEqual(bench.get_metadata()["hooks"], "_test_hook")
...
class TestRunnerCPUAffinity(unittest.TestCase):
    ...
    def test_cpu_affinity_isolcpus(self):
        ...
        self.assertEqual(stdout.getvalue(), "Pin process to isolated CPUs: 1-2\n")

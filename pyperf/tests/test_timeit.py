...
FAST_BENCH_ARGS = ("--debug-single-value", "-s", "import time", "time.sleep(1e-6)")
FAST_MIN_TIME = 1e-6
# test with a least with two values
COMPARE_BENCH = ("-l1", "-p1", "-w0", "-n3", "-s", "import time", "time.sleep(1e-6)")
...
class TestTimeit(unittest.TestCase):
    ...
    def test_worker_verbose(self):
        ...
        match = re.search(
            r"Warmup 1: ([0-9.]+) ms \(loops: 1, raw: [0-9.]+ ms\)\n"
            r"\n"
            r"Value 1: ([0-9.]+) ms\n"
            r"Value 2: ([0-9.]+) ms\n"
            r"\n"
        ...
        for value in values:
            self.assertTrue(MIN_VALUE <= value <= MAX_VALUE, repr(value))
    ...
    def test_cli(self):
        ...
        match = re.search(
            r"Mean \+- std dev: (?P<mean>[0-9.]+) ms" r" \+- (?P<mad>[0-9.]+) ms" r"$",
            cmd.stdout.rstrip(),
        )
        self.assertIsNotNone(match, repr(cmd.stdout))
    ...
    @unittest.skipIf(
        pyperf.python_implementation() == "pypy", "pypy program cannot be copied"
    )
    @unittest.skipIf(sys.platform == "win32", "https://github.com/psf/pyperf/issues/97")
    def test_python_option(self):
        ...
        try:
            ...
            args = ("--metadata", "--python", tmp_exe, "--inherit-env", "PYTHONPATH")
    ...
    def test_teardown_multi_line(self):
        args = (
            PERF_TIMEIT
            + ("--teardown", "assert 2 == 2", "--teardown", "assert 2 == 2")
            + FAST_BENCH_ARGS
        )
        cmd = tests.get_output(args)
...
class TimerTests(unittest.TestCase):
    ...
    def test_returns_valid_template_with_all_callable_params(self):
        ...
        output = template_output(
            ...
            stmt="_stmt()",
            teardown="_teardown()",
            init=", _setup=_setup, _stmt=_stmt, " "_teardown=_teardown",
        )
        self.assertEqual(timer.src, output)
        self.assertDictEqual(
            {"_setup": setup, "_stmt": stmt, "_teardown": teardown}, timer.local_ns
        )

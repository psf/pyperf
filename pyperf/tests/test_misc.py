...
class MiscTests(unittest.TestCase):
    def test_version_tuple(self):
        ...
        self.assertTrue(
            all(isinstance(part, int) for part in pyperf.VERSION), pyperf.VERSION
        )

    def test_version_str(self):
        self.assertIsInstance(pyperf.__version__, str)
        self.assertEqual(
            pyperf.__version__, ".".join(str(part) for part in pyperf.VERSION)
        )

    def test_format_metadata(self):
        self.assertEqual(pyperf.format_metadata("loops", 2**24), "2^24")
    ...
    def test_abs_executable(self):
        with tests.temporary_file() as tmpname:
            ...
            self.assertEqual(utils.abs_executable(tmpname), tmpname)
    ...
    def test_parse_run_list(self):
        ...
        with self.assertRaises(ValueError):
            parse_run_list("0")
        self.assertEqual(parse_run_list("1"), [0])
        self.assertEqual(parse_run_list("1-2,5-6"), [0, 1, 4, 5])
        self.assertEqual(parse_run_list("1,3,7"), [0, 2, 6])

        # tolerate spaces
        self.assertEqual(parse_run_list(" 1 , 2 "), [0, 1])

...
def format_checks(bench, lines=None, check_too_many_processes=False):
    ...
    if len(values) >= 2:
        ...
        if percent >= 10.0:
            ...
            if required_nprocesses is not None and required_nprocesses > len(
                bench._runs
            ):
                warn(
    ...
    if warnings:
        ...
        for msg in warnings:
            lines.append("* %s" % msg)
        empty_line(lines)
        lines.append("Try to rerun the benchmark with more runs, values and/or loops.")
        lines.append(
    ...
    if check_too_many_processes:
        if required_nprocesses is None:
            required_nprocesses = bench.required_nprocesses()
        if (
            required_nprocesses is not None
            and required_nprocesses < len(bench._runs) * 0.75
        ):
            lines.append(

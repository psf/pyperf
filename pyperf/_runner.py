...
class Runner:
    ...
    def __init__(
        ...
        parser.add_argument(
            ...
            default=processes,
            help="number of processes used to run benchmarks (default: %s)" % processes,
        )
        parser.add_argument(
            "-n",
            "--values",
        ...
        parser.add_argument(
            ...
            help="Python executable (default: use running Python, sys.executable)",
        )
        parser.add_argument(
            "--compare-to",
            metavar="REF_PYTHON",
        ...
        parser.add_argument(
            "--profile",
            type=str,
            help="Collect profile data using cProfile and output to the given file.",
        )
    ...
    def _process_args_impl(self):
        ...
        if args.calibrate_loops:
            ...
            if args.worker and args.warmups is None:
                raise CLIError("--worker requires --warmups=N or --calibrate-warmups")
        ...
        if args.compare_to:
            for option in ("output", "append"):
                if getattr(args, option):
                    raise CLIError(
                        "--%s option is incompatible with --compare-to option" % option
                    )
